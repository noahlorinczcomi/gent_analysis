...
#  - Code: GATES(pvalues,ld_matrix) (see below)
#  - Example usage: GATES(pvals,ld_matrix)
# 
# GenT
...
meff=function(lambda,M=length(lambda)) M-sum(+(lambda>1)*(lambda-1))
...
for(i in 1:length(ms)) {
  ...
  for(j in 1:length(rhos)) {
    ...
    b=rep(0,m);b[round(m/2)]=sqrt(0.0005/3*50000);Z0=LD0%*%b # 0.1% heritability explained, 3 causal SNPs per gene, GWAS n=50K
    Z=mvnfast::rmvn(niter,Z0,LD0)
    Z2=Z^2
    ...
    for(iter in 1:niter) {
      ...
      # exset
      t0=Sys.time()
      res2[iter]=exset(sum(Z2[iter,]),ld_eigenvalues,iterations=1e5,type1_error=0.05)=='reject H0'
      ...
      # snpsettest (VEGAS)
      t0=Sys.time()
      res4[iter]=snpsettest(Z2[iter,],ld_eigenvalues,lower.tail=FALSE)<0.05
